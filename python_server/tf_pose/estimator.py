    ...
    def estimate_paf(peaks, heat_mat, paf_mat):
        ...
        for human_id in range(pafprocess.get_num_humans()):
            ...
            if is_added:
                ...
                human.score = score
                if len(humans) == 0 or humans[0].score < score:
                    humans = [human]
...
class TfPoseEstimator:
    ...
    @staticmethod
    def get_pose_fit_score(humans, reference):
        # reference is last frame of first video
        part_name = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist", "LShoulder", "LElbow", "LWrist", "RHip",
                     "RKnee", "RAnkle", "LHip", "LKnee", "LAnkle", "REye", "LEye", "REar", "LEar"]

        if reference is None or len(humans) != 1:
            return float("inf")

        # reference for body.
        if part_name.index("Nose") not in humans[0].body_parts or part_name.index("Neck") not in humans[0].body_parts:
            return float("inf")

        human = copy.deepcopy(humans[0])

        ref_dict = reference.body_parts
        frame_dict = human.body_parts

        def get_distance(x1, x2, y1, y2):
            return ((x1-x2) ** 2 + (y1-y2) **2)  ** 0.5

        # eval ref points & distance
        base_x_ref = ref_dict[1].x
        base_y_ref = ref_dict[1].y
        base_dist_ref = get_distance(ref_dict[1].x, ref_dict[0].x, ref_dict[1].y, ref_dict[0].y)
        base_x_frame = frame_dict[1].x
        base_y_frame = frame_dict[1].y
        base_dist_frame = get_distance(frame_dict[1].x, frame_dict[0].x, frame_dict[1].y, frame_dict[0].y)

        # ref_frame_ratio = base_dist_ref / base_dist_frame

        total_diff = 0
        part_count = 0
        for part_index in range(len(part_name)):
            if part_index not in ref_dict or part_index not in frame_dict:
                continue

            raw_ref_x, raw_ref_y = ref_dict[part_index].x, ref_dict[part_index].y
            ref_x = (raw_ref_x - base_x_ref) / base_dist_ref
            ref_y = (raw_ref_y - base_y_ref) / base_dist_ref

            raw_frame_x, raw_frame_y = frame_dict[part_index].x, frame_dict[part_index].y
            frame_x = (raw_frame_x - base_x_frame) / base_dist_frame
            frame_y = (raw_frame_y - base_y_frame) / base_dist_frame

            diff = get_distance(ref_x, frame_x, ref_y, frame_y)
